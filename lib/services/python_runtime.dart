...
/// поэтому обычный вывод повторяет всё сначала. Пока выполнение не зашло
/// дальше, чем в прошлый раз, новый stdout является расширением старого
/// префикса — показываем только добавленные строки, и консоль растёт,
/// как настоящая.
///
/// Случаи:
/// * новый вывод расширяет старый → только суффикс;
/// * новый вывод — уже виденный префикс (например, новые данные сразу привели
///   к `break`) → ничего нового, старый показ не трогаем;
/// * вывод изменился с начала (например, `random`) → отдаём всё целиком.
